...
query_limit = [200, 248, 300, 300, 7405, 6666]
if(num_query == -1):
...
def llm_eval_run(
    ...
    if(not no_generation):
        ...
        model = NanoLLM.from_pretrained(model=llm_model_local, api='mlc', quantization='q4f16_ft')
    ...
    # Settings.embed_model = HuggingFaceEmbedding(model_name=embedding_model_path, trust_remote_code=True, embed_batch_size = 32, fp16=True)
    Settings.embed_model = HuggingFaceEmbedding(model_name=embedding_model_path, trust_remote_code=True, embed_batch_size = 32)
    # Load index
    index_dir = os.path.join(index_path_local, "index", dataset + "_" + str(chunk_size)+"_"+index_type)
    if(index_type=='ivfFlat'):
        index_dir = os.path.join(index_dir+"_"+str(ivf_num_cell))
        print("Using ivfFlat: " + index_dir)
    if(not os.path.exists(index_dir)):
        print("Index does not exist, exiting")
        print("Please check the index path: " + index_dir)
        exit()
    # Load vectorstore
    ...
    dataset_full_path = os.path.join(dataset_path, "datasets", "BeIR__" + dataset)
    ds_path  = os.path.join(dataset_full_path, dataset)
...
def llm_eval_run2(
    ...
    # Load Embedding model
    from llama_index.embeddings.huggingface import HuggingFaceEmbedding
    # embed_model = HuggingFaceEmbedding(model_name=embedding_model_path, trust_remote_code=True, embed_batch_size = 32, fp16=True)
    embed_model = HuggingFaceEmbedding(model_name=embedding_model_path, trust_remote_code=True, embed_batch_size = 32)
    Settings.embed_model = embed_model
    # Load index
    import pickle
    index_dir = os.path.join(index_path_local, "index", dataset + "_two_level")
    ...
    cache_state = None
    if(load_cache):
        cache_path = os.path.join(index_path_local, "cache") # 这里存疑，缓存是否需要在内存中，要的话这里需要把路径改成一个磁盘路径
        ...
        if(not os.path.exists(cache_dict_name)):
            ...
            print("Invalid cache dir: " + cache_dict_name)
            exit()
    ...
    if(load_cache_checkpoint):
        cache_path = os.path.join(index_path_local, "cache_temp") # 这里存疑，缓存是否需要在内存中，要的话这里需要把路径改成一个磁盘路径
    ...
    if(config_idx == 3):
        ...
        if(load_cache):
            if(alg == 1):
                ...
                retriever = TwoStageRetrieverCachedFilesize(faiss_index=index, corpus_list=corpus,
                    embed_model=embed_model, redir_table=redir_table, index_dir=index_dir,
                    cost_table=cost_table, cache_obj=cache_state, max_probe_cost=max_probe_cost,
                    cache_max_size=cache_max_size, min_cache_cost = min_cache_cost)
            elif(alg == 3):
                retriever = TwoStageRetrieverCachedLLB(faiss_index=index, corpus_list=corpus,
                    embed_model=embed_model, redir_table=redir_table, index_dir=index_dir,
                    cost_table=cost_table, cache_obj=cache_state, max_probe_cost=max_probe_cost,
                    cache_max_size=cache_max_size, min_cache_cost = min_cache_cost)
            elif(alg == 4):
    ...
    if(llm_evaluation):
        ...
        gpt4 = DeepSeek(model="deepseek-chat", api_key="sk-b330861fb4fa4c03adb1b14ae623b80c")
        evaluator_gpt4 = RelevancyEvaluator(llm=gpt4)
    ...
    for key, query in queries.items():
        ...
        print("Q_STATS, Retrieve Time: " + str(retrieve_time) + ", Prefill Latency: " + str(first_token_gen_time) + ", Generation Latency: " + str(generation_time) + ", Generated Tokens: " + str(token_count))
        query_counter += 1
        print("Query counter: " + str(query_counter) + "/" + str(query_limit))
        if(query_counter>=query_limit):
            break
    if(config_idx>=5):
    ...
    if(prefetch):
        ...
        cache_dict_file.close()
        print("prefetch to: " + cache_dict_name)
    ...
    if(save_cache_checkpoint):
        ...
        pickle.dump(retriever.get_cache_dict(), cache_dict_file)
        cache_dict_file.close()
        print("cache_checkpoint to: " + cache_dict_name)
